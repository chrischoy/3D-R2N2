    ...
    def __init__(self, w_shape, is_bias, mean=0, std=0.01, filler='msra',
        ...
        if isinstance(w_shape, collections.Iterable) and not is_bias:
            if len(w_shape) > 1 and len(w_shape) < 5:
                ...
                raise NotImplementedError('Filter shape with ndim > 5 not supported: len(w_shape) = %d' % len(w_shape))
        else:
            n = 1
        ...
        if filler == 'gaussian':
            ...
            self.np_values = np.asarray(rng.normal(mean, np.sqrt(2. / n),
                                        w_shape), dtype=theano.config.floatX)
        elif filler == 'xavier':
            scale = np.sqrt(3. / n)
            self.np_values = np.asarray(rng.uniform(low=-scale, high=scale,
                                 size=w_shape), dtype=theano.config.floatX)
...
class BlockDiagonalLayer(Layer):
    ...
    def __init__(self, prev_layer, n_out, params=None, bias=True):
        ...
        if params is None:
            ...
            if bias:
                self.b = Weight(self._output_shape[1:], is_bias=True, mean=0.1,
                                filler='constant')
        else:
            self.W = params[0]
            if bias:
...
class ReshapeLayer(Layer):
    ...
    def set_output(self):
        self._output = tensor.reshape(self._prev_layer.output,
                                      self._output_shape)
    ...
    def set_output(self):
        if sum(self._padding) > 0:
            ...
            padded_input = tensor.set_subtensor(
                padded_input[:,
                             :,
                             self._padding[2]:self._padding[2] + self._input_shape[2],
                             self._padding[3]:self._padding[3] + self._input_shape[3]],
                self._prev_layer.output)

            padded_input_shape = [self._input_shape[0],
                                  self._input_shape[1],
                                  self._input_shape[2] + 2 * self._padding[2],
                                  self._input_shape[3] + 2 * self._padding[3]]
        else:
        ...
        conv_out = conv.conv2d(
            ...
            image_shape=np.asarray(padded_input_shape, dtype=np.int16),
            border_mode='valid'
        )
...
class FCConv3DLayer(Layer):
    ...
    def __init__(self, prev_layer, fc_layer, filter_shape,
                 padding=None, params=None):
        ...
        if padding is None:
            self._padding = [0,
                             int((self._filter_shape[1] - 1) / 2),
                             0,
                             int((self._filter_shape[3] - 1) / 2),
                             int((self._filter_shape[4] - 1) / 2)]
        ...
        if params is None:
            ...
            self._Wx_shape = [self._fc_layer._output_shape[1],
                              np.prod(self._output_shape[1:])]
            ...
            self.Wx = Weight(self._Wx_shape, is_bias=False,
    ...
    def set_output(self):
        ...
        fc_output = tensor.reshape(tensor.dot(self._fc_layer.output,
                                              self.Wx.val),
                                   self._output_shape)
...
class SoftmaxWithLoss3D(object):
    ...
    def loss(self, y):
        ...
        label.
        """
        return tensor.mean(tensor.sum(- y * self.input, axis=2, keepdims=True)
                           + tensor.log(self.sum_exp_x))
...
#     """
#     For a given input whose first dimension is the index of the data, return
#     averaged L2 distance between y and input.
#     """
#     def __init__(self, input):
#         self.input = input
#
#     def loss(self, y):
#         assert(self.input.ndim == y.ndim)
#         # flatten from the second axis
#         diff = tensor.sqr(self.input.flatten(2) - y.flatten(2))
#         # return tensor.mean(tensor.sum(diff, axis=1))
#         return tensor.mean(tensor.sqrt(tensor.sum(diff, axis=1)))

# TODO
# class BatchNormalization(object):
#     def __init__(self, input, input_shape, pattern=['x',0,1,2]):
...
#         )
#
#         mean = input.mean(axis=0, keepdims=False)
#         var = input.var(axis=0, keepdims=False)
#
#         self.output = batch_normalization(input,
#                                           self.gamma.dimshuffle(*pattern),
#                                           self.beta.dimshuffle(*pattern),
#                                           mean.dimshuffle(*pattern),
#                                           tensor.sqrt(var.dimshuffle(*pattern) + 1e-7),
#                                           mode='high_mem')
...
#         m = input_shape[0]
#         adjust = m/(m-1)
#         self.updates = [(self.avg_mean, self.avg_mean*decay + (1-decay)*mean),
#                         (self.avg_var, self.avg_var*decay + (1-decay)*adjust*var)]
#         self.params_save = [self.avg_mean, self.avg_var]


# class InceptionConvModule(object):
#     def __init__(self, input, input_shape,
#                  reduce_channels, out_channels,
#                  non_linearity=None):
#
#         assert(len(out_channels)==3 and len(reduce_channels)==2)
#
#         path0_conv = ConvLayer(
#             input=input,
#             input_shape=input_shape,
#             filter_shape=[out_channels[0], input_shape[1], 1, 1]
#         )
#         path0_conv_rect = non_linearity(path0_conv.output) if non_linearity \
#             else path0_conv
#
#         path1_reduce = ConvLayer(
#             input=input,
#             input_shape=input_shape,
#             filter_shape=[reduce_channels[0], input_shape[1], 1, 1]
#         )
#         path1_reduce_rect = non_linearity(path1_reduce.output) if non_linearity \
#             else path1_reduce
#
#         path1_conv = ConvLayer(
#             input=path1_reduce_rect.output,
#             input_shape=path1_reduce.output_shape,
#             filter_shape=[out_channels[1], reduce_channels[0], 3, 3]
#         )
#         path1_conv_rect = non_linearity(path1_conv.output) if non_linearity \
#             else path1_conv
#
#         path2_reduce = ConvLayer(
#             input=input,
#             input_shape=input_shape,
#             filter_shape=[reduce_channels[1], input_shape[1], 1, 1]
#         )
#         path2_reduce_rect = non_linearity(path2_reduce.output) if non_linearity \
#             else path2_reduce
#
#         path2_conv = ConvLayer(
#             input=path2_reduce_rect.output,
#             input_shape=path2_reduce.output_shape,
#             filter_shape=[out_channels[2], reduce_channels[1], 5, 5]
#         )
#         path2_conv_rect = non_linearity(path2_conv.output) if non_linearity \
#             else path2_conv
#
#         concat_depth = out_channels[0] + out_channels[1] + out_channels[2]
#         self.output = tensor.concatenate([path0_conv_rect.output,
#                                           path1_conv_rect.output,
#                                           path2_conv_rect.output], axis=1)
#         self.params = path0_conv.params + path1_reduce.params + \
#                       path1_conv.params + path2_reduce.params + \
#                       path2_conv.params
#         self.output_shape = [input_shape[0], concat_depth,
#                              input_shape[2], input_shape[3]]
#
#
# class InceptionConv3DModule(object):
#     def __init__(self, input, input_shape,
#                  reduce_channels, out_channels,
#                  non_linearity=None):
#
#         assert(len(out_channels)==3 and len(reduce_channels)==2)
#
#         path0_conv = Conv3DLayer(
#             input_x=input,
#             input_shape=input_shape,
#             filter_shape=[out_channels[0], 1, input_shape[2], 1, 1]
#         )
#         path0_conv_rect = non_linearity(path0_conv.output) if non_linearity \
#             else path0_conv
#
#         path1_reduce = Conv3DLayer(
#             input_x=input,
#             input_shape=input_shape,
#             filter_shape=[reduce_channels[0], 1, input_shape[2], 1, 1]
#         )
#         path1_reduce_rect = non_linearity(path1_reduce.output) if non_linearity \
#             else path1_reduce
#
#         path1_conv = Conv3DLayer(
#             input_x=path1_reduce_rect.output,
#             input_shape=path1_reduce.output_shape,
#             filter_shape=[out_channels[1], 3, reduce_channels[0], 3, 3]
#         )
#         path1_conv_rect = non_linearity(path1_conv.output) if non_linearity \
#             else path1_conv
#
#         path2_reduce = Conv3DLayer(
#             input_x=input,
#             input_shape=input_shape,
#             filter_shape=[reduce_channels[1], 1, input_shape[2], 1, 1]
#         )
#         path2_reduce_rect = non_linearity(path2_reduce.output) if non_linearity \
#             else path2_reduce
#
#         path2_conv = Conv3DLayer(
#             input_x=path2_reduce_rect.output,
#             input_shape=path2_reduce.output_shape,
#             filter_shape=[out_channels[2], 5, reduce_channels[1], 5, 5]
#         )
#         path2_conv_rect = non_linearity(path2_conv.output) if non_linearity \
#             else path2_conv
#
...
#
#
# class ConvPoolModule(object):
#     def __init__(self, input, input_shape, filter_shape, pool_size,
#                  do_padding=True):
#         conv = ConvLayer(
#             input=input,
#             input_shape=input_shape,
...
#         )
#
#         rect = LeakyReLU(pool.output).output
#         self.output = rect
#         self.params = conv.params
#         self.output_shape = pool.output_shape
#
#
# class UnpoolConv3DModule(object):
#     def __init__(self, input, input_shape, filter_shape, unpool_size):
#         unpool = Unpool3DLayer(
#             input,
#             input_shape,
...
#
#         conv = Conv3DLayer(
#             input_x=unpool.output,
#             input_shape=unpool.output_shape,
#             filter_shape=filter_shape
#         )
#
#         rect = LeakyReLU(conv.output).output
#         self.output = rect
#         self.params = conv.params
