...
from lib.data_process import ReconstructionDataProcess, make_data_processes, get_while_running
...
def test_net():
    ...
    processes = make_data_processes(queue, data_pair, 1, train=False)
    ...
    # Get all test data
    for batch_img, batch_voxel in get_while_running(queue):
        # There can be less than batchsize number of data
        n = len(batch_voxel)
