...
class RecNet(Net):
    def network_definition(self):
        ...
        def recurrence(x_curr, prev_s_tensor, update_gate_tensor, conv11_tensor):
            ...
            unpool7_ = Unpool3DLayer(gru_out_)
            conv7_   = Conv3DLayer(unpool7_, (n_deconvfilter[1], 3, 3, 3), params=conv7.params)
            rect7_   = LeakyReLU(conv7_)
            unpool8_ = Unpool3DLayer(rect7_)
            conv8_   = Conv3DLayer(unpool8_, (n_deconvfilter[2], 3, 3, 3), params=conv8.params)
            rect8_   = LeakyReLU(conv8_)
            unpool9_ = Unpool3DLayer(rect8_)
            conv9_   = Conv3DLayer(unpool9_, (n_deconvfilter[3], 3, 3, 3), params=conv9.params)
            rect9_   = LeakyReLU(conv9_)
            conv10_  = Conv3DLayer(rect9_, (n_deconvfilter[4], 3, 3, 3), params=conv10.params)
            rect10_  = LeakyReLU(conv10_)
            conv11_  = Conv3DLayer(rect10_, (n_deconvfilter[5], 3, 3, 3), params=conv11.params)

            return gru_out_.output, update_gate_.output, conv11_.output

        s_update_pred, _ = theano.scan(recurrence,
            sequences=[self.x],  # along with images, feed in the index of the current frame
            outputs_info=[tensor.zeros_like(np.zeros(s_shape), dtype=theano.config.floatX),
                          tensor.zeros_like(np.zeros(s_shape), dtype=theano.config.floatX),
                          tensor.zeros_like(np.zeros((self.batch_size, n_vox, 2, n_vox, n_vox)), dtype=theano.config.floatX)])

        update_all = s_update_pred[1]
        pred_all = s_update_pred[2]
        softmax_loss = SoftmaxWithLoss3D(pred_all[-1])
        ...
        self.activations = [update_all, pred_all]
